...
#include "nop_delay.h"
#ifndef __CYGWIN__
//Because the error is introduced as a header file, tony_misc.c is directly introduced to 		author:hh
...
// Introduce global variables to calculate thread runtime	author：hh
struct thread_data*  thread_param;
// Introduce a global variable to pass in the number of lock_counts for a single thread		author：hh
...
void *thread_routine(void *arg)
{
    ...
    */
    ...
    while(1)
	{
		...
		parg->count++;// Add count variable	author：hh
	}	 
	/*
    int i = 0;
    while(i<1000){
...
void sig_handler(int signo)
{
        ...
        {			
			    ...
			    printf("running time:%lld\n",(tm_ended.tv_sec-tm_start.tv_sec)*1000000000+tm_ended.tv_nsec-tm_start.tv_nsec);    // Output detailed running time author：hh
                exit(0);
        }
}
...
int main(int argc, char *argv[])
{
        ...
        int i=0;
        for( i = 0; i < nr_parallel_threads; ++i) {
                thread_param[i].cpu_id = i % get_nprocs();//This code has no effect because it is not tied to the core.		author：hh
